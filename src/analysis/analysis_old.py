...
# PCA Analysis 
# Parameters include the dataset received from previous stage:______, feature coluumns, label column and n_components
def pca_analysis(df, features, label, n_components=2):
    
    X = prepare_training_features(df, features) # uses median but can add more options
    ...
    result = {"pca_df": pca_df, "features": features, "explained_variance_ratio": pca.explained_variance_ratio_, "explained_variance_sum": pca.explained_variance_ratio_.sum()}
...
def mutual_information_analysis(df, features, label):
    X = prepare_training_features(df, features) # uses median but can add more options
    y = df[label]
    ...
    result = pd.DataFrame({"feature": features, "mutual_information": mi_scores}).sort_values("mutual_information", ascending=False)

    return result

# mRMR (minimum Redundancy - Maximum Relevance) Analysis
def mrmr_analysis(df, features, label, K=10):
    ...
    result = pd.DataFrame({"rank": rank, "feature": top_features})
